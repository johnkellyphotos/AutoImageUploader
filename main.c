...
void download_folder(const char *folder) {
  ...
  if (pid == 0) {
    char filepath[512];
    snprintf(filepath, sizeof(filepath), "%s/%%f_%%Y%%m%%d-%%H%%M%%S.jpg",
             LOCAL_DIR);

    execlp("gphoto2", "gphoto2", "--get-all-files", "--new", "--skip-existing",
           "--folder", folder, "--filename", filepath, NULL);
    _exit(1);
  } else if (pid > 0) {
    int status;
...
void download_existing_files() {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (pid == 0) {
      ...
      fflush(stdout);
      char filepath[512];
      snprintf(filepath, sizeof(filepath), "%s/%%f_%%Y%%m%%d-%%H%%M%%S_%%C.jpg",
               LOCAL_DIR);

      execlp("gphoto2", "gphoto2", "--get-all-files", "--new",
             "--skip-existing", "--folder", fullpath, "--filename", filepath,
             NULL);
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    if (gphoto_pid <= 0) {
      ...
      gphoto_pid = fork();
      if (gphoto_pid == 0) {
        execlp("gphoto2", "gphoto2", "--wait-event-and-download",
               "--skip-existing", "--folder", "/", "--filename",
               "/home/john/import/%f_%Y%m%d-%H%M%S_%C.jpg", NULL);
